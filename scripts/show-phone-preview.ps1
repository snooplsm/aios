...
if ($apiLevel -lt 35) {
    throw "AIOS Phone preview requires Android API 35 or newer"
}
...
if (-not $SkipBuild) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "AIOS Phone preview build failed"
        }
    } finally {
        Pop-Location
    }
...
Invoke-Adb shell am start -n com.aios.phone.preview/.PreviewActivity `
    --es aios_preview_scenario $Scenario `
    --es aios_preview_theme $Theme | Out-Null

$expectedText = @{
    home = "Phone number"
    recents = "Potential spam"
...
$remoteScreenshot = "/sdcard/aios-phone-preview-$token.png"
...
try {
    ...
    if ($Scenario -eq "incoming") {
        ...
        if ($ai.Count -ne 1 -or $null -eq $ai[0].ParentNode -or
            ...
            throw "Incoming visual scenario did not expose an enabled AI action"
        }
    }

    if ([string]::IsNullOrWhiteSpace($Screenshot)) {
        $Screenshot = Join-Path $previewRoot (
            "screenshots\aios-preview-$Scenario-$Theme.png")
    }
    ...
    if (-not (Test-Path -LiteralPath $screenshotPath) -or
        ...
        throw "Preview screenshot was not captured"
    }
    Write-Output "AIOS Phone preview ready: scenario=$Scenario theme=$Theme"
