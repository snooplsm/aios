...
if ($LASTEXITCODE -ne 0 -or $sourceRevision -notmatch '^[0-9a-f]{40}$') {
    throw "Unable to bind model-admission evidence to an exact AIOS revision"
}
...
if ($unstagedChanges -ne 0 -or $stagedChanges -ne 0) {
    throw "Refusing to capture model-admission evidence with tracked source changes"
}
...
try {
    ...
    $evidence = [ordered]@{
        schema_version = 1
        gate = "integration.emulator_model_admission"
